...
//2-sub(Pricing strategy)
class PricingStrategy {
    public:
        virtual ~PricingStrategy() {}
        virtual double calculateFee(time_t entryTime, time_t exitTime, VehicleType type) const = 0;
};

class VehicleBasedHourlyStrategy: public PricingStrategy {
    public:
        double calculateFee(time_t entryTime, time_t exitTime, VehicleType type) const override {
...
class ExitGate: public Gate {
    private:
        ...
        public:
            ExitGate(string id, PricingStrategy* strategy): Gate(id), pricingStrategy(strategy) {}
            ...
            double processExit(Ticket* ticket){
                ...
                double fee = pricingStrategy->calculateFee(ticket->getEntryTime(), exitTime, vehicle->getType());
